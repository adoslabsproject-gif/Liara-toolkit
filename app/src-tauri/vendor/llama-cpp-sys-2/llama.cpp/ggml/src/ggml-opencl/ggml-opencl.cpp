...
#include <CL/cl.h>
#include <dlfcn.h>   // dlsym: carica clCreateBufferWithProperties (OpenCL 3.0) lazy → compat GPU Mali
...
static ggml_backend_buffer_t ggml_backend_opencl_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buffer_type, size_t size) {
    ...
    if (err != CL_SUCCESS && backend_ctx->adreno_use_large_buffer) {
        ...
        // clCreateBufferWithProperties è OpenCL 3.0: i driver Mali (OpenCL 2.0 — Oppo/Redmi MediaTek)
        // NON esportano il simbolo, e linkarlo STATICAMENTE fa fallire il dlopen dell'INTERA libreria
        // su quei device (UnsatisfiedLinkError → crash all'avvio, senza nemmeno arrivare al codice).
        // Lo carichiamo via dlsym (lazy): su Adreno c'è → large-buffer ok; su Mali è NULL ma qui non
        // si entra comunque (adreno_use_large_buffer=false). Così la libreria si carica su QUALSIASI GPU.
        typedef cl_mem (*pfn_clCBWP)(cl_context, const cl_mem_properties *, cl_mem_flags, size_t, void *, cl_int *);
        static pfn_clCBWP p_clCBWP = (pfn_clCBWP) dlsym(RTLD_DEFAULT, "clCreateBufferWithProperties");
        if (p_clCBWP) {
            mem = p_clCBWP(backend_ctx->context, props, CL_MEM_READ_WRITE, size, NULL, &err);
        }
    }
