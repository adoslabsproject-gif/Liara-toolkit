...
// `static inline` (non solo `static`): miniaudio è un single-header, alcune sue funzioni pubbliche
// non vengono usate in questa TU → con `static` sole davano -Wunused-function (le ma_dr_mp3_*).
// `inline` è lo storage corretto per funzioni definite in header: le inutilizzate NON avvisano più.
// Fix VERO (non -Wno-): il codice inutilizzato resta compilabile, semplicemente non genera warning.
