...
static inline projector_type clip_projector_type_from_string(const std::string & str) {
...
static inline void clip_log_callback_default(enum ggml_log_level level, const char * text, void * user_data) {
    (void) level;
    (void) user_data;
    fputs(text, stderr);
    fflush(stderr);
...
static inline void clip_log_internal(enum ggml_log_level level, const char * format, ...) {
    va_list args;
...
static inline std::vector<std::string> string_split_str(std::string s, const std::string & delimiter) {
...
static inline std::string gguf_kv_to_str(const struct gguf_context * ctx_gguf, int i) {
...
static inline void print_tensor_shape(ggml_tensor * t) {
...
static inline void print_tensor_data(ggml_tensor * t, uint8_t * data, int64_t n) {
