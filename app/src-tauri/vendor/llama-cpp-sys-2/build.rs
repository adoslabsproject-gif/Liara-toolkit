...
fn main() {
    ...
    // macOS: ggml usa std::filesystem::path (richiede deployment target >= 10.15). cmake-rs NON propaga
    // MACOSX_DEPLOYMENT_TARGET ai .cpp → lo forziamo qui, altrimenti la compilazione di ggml fallisce.
    #[cfg(target_os = "macos")]
    config.define("CMAKE_OSX_DEPLOYMENT_TARGET", "11.0");
    // ggml-backend-dl (caricamento DINAMICO dei backend) usa std::filesystem (richiede 10.15+). Su macOS
    // i backend (Metal/CPU) sono linkati STATICAMENTE → lo disabilitiamo, evitando quei .cpp problematici.
    #[cfg(target_os = "macos")]
    ...
    if matches!(target_os, TargetOs::Android) {
        ...
        // Allinea le sezioni ELF a 16KB così libapp_lib.so non compare nell'avviso Samsung
        // "librerie non allineate" (e resta compatibile coi futuri device a pagine da 16KB).
        println!("cargo:rustc-link-arg=-Wl,-z,max-page-size=16384");
        println!("cargo:rustc-link-arg=-Wl,-z,common-page-size=16384");
        // GGML_OPENMP=OFF: su Android l'OpenMP (libomp) ha una race di un mutex statico distrutto
        // ("destroyed mutex") durante l'init dei thread → crash intermittente all'avvio. La doc
        // Snapdragon di llama.cpp raccomanda OFF: ggml usa il suo thread pool, niente libomp, stabile.
        ...
        // GPU via Vulkan (Adreno/Mali): accende GGML_VULKAN e indica glslc (nell'NDK). Gli header e
        // libvulkan.so per Android sono nel sysroot dell'NDK → trovati via il toolchain file. Gated
        // da LIARA_VULKAN per poter tornare alla CPU se servisse.
        println!("cargo:rerun-if-env-changed=LIARA_VULKAN");
        if env::var("LIARA_VULKAN").is_ok() {
            config.define("GGML_VULKAN", "ON");
            // ggml-vulkan usa vkGetInstanceProcAddr da libvulkan → DEVE essere linkato esplicitamente
            // (altrimenti UnsatisfiedLinkError al caricamento). libvulkan.so è nel sysroot NDK.
            println!("cargo:rustc-link-lib=dylib=vulkan");
            let glslc = format!("{}/shader-tools/darwin-x86_64/glslc", android_ndk);
            config.define("Vulkan_GLSLC_EXECUTABLE", &glslc);
            let manifest = env::var("CARGO_MANIFEST_DIR").unwrap();
            // SPIRV-Headers recenti (vendor/SPIRV-Headers) — quelli dell'NDK sono troppo vecchi
            // (manca unified1/spirv.hpp). Generiamo il config cmake + li aggiungiamo agli include.
            let spirv_inc = format!("{manifest}/../SPIRV-Headers/include");
            let cfg_dir = PathBuf::from(env::var("OUT_DIR").unwrap()).join("spirv-headers-cmake");
            std::fs::create_dir_all(&cfg_dir).unwrap();
            std::fs::write(
                cfg_dir.join("SPIRV-HeadersConfig.cmake"),
                format!(
                    "add_library(SPIRV-Headers::SPIRV-Headers INTERFACE IMPORTED)\n\
                     set_target_properties(SPIRV-Headers::SPIRV-Headers PROPERTIES \
                     INTERFACE_INCLUDE_DIRECTORIES \"{spirv_inc}\")\n"
                ),
            )
            .unwrap();
            config.define("SPIRV-Headers_DIR", cfg_dir.to_str().unwrap());
            // L'NDK ha solo gli header C di Vulkan; ggml-vulkan usa quelli C++ (Vulkan-Hpp). Li
            // forniamo da vendor/Vulkan-Headers; più gli SPIRV-Headers recenti. Tutti come -I.
            let vk_hpp_inc = format!("{manifest}/../Vulkan-Headers/include");
            for inc in [&vk_hpp_inc, &spirv_inc] {
                config.cflag(format!("-I{inc}"));
                config.cxxflag(format!("-I{inc}"));
            }
        }

        // GPU via OpenCL (backend Qualcomm per Adreno): i kernel li compila il driver Adreno a
        // runtime (niente shader pre-compilati) → più robusto del Vulkan sull'Adreno.
        println!("cargo:rerun-if-env-changed=LIARA_OPENCL");
        if env::var("LIARA_OPENCL").is_ok() {
            let manifest = env::var("CARGO_MANIFEST_DIR").unwrap();
            config.define("GGML_OPENCL", "ON");
            config.define("GGML_OPENCL_USE_ADRENO_KERNELS", "ON"); // kernel ottimizzati Adreno
            config.define("GGML_OPENCL_TARGET_VERSION", "300");
            // OpenCL-Headers + libOpenCL (presa dal device) forniti da vendor/.
            config.define("OpenCL_INCLUDE_DIR", format!("{manifest}/../OpenCL-Headers"));
            config.define("OpenCL_LIBRARY", format!("{manifest}/../opencl-lib/libOpenCL.so"));
            println!("cargo:rustc-link-search=native={manifest}/../opencl-lib");
